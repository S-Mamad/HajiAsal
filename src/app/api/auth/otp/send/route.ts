...
export async function POST(request: Request) {
  try {
    ...
    return withOtpLock(`otp-send:${phone}`, async () => {
      ...
      // Reserve cooldown before the SMS leaves so parallel tabs cannot double-send.
      ...
      const dispatched = await dispatchOtpSend(phone, phone);
      if (!dispatched.ok) {
        return NextResponse.json(
      ...
      void (async () => {
        try {
          ...
          if (!rl.ok) return;
          await enqueueTelegramAlert("auth.otp_requested", { phone });
        } catch (err) {
          console.error(
            "[otp/send] telegram",

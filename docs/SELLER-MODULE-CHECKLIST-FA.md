...
> وضعیت اجرا (۱۸ جولای ۲۰۲۶): **Products** و **Orders** به‌عنوان الگوی کامل (لیست + فیلتر ذخیره‌شده + selection/bulk + export/print + history) پاس شده‌اند. Inventory (تاریخچه حرکات)، Media (آپلود واقعی)، Wallet ادمین (تأیید برداشت)، و اتصال کد تخفیف فروشنده به `/api/coupons` هم اضافه شد. بقیه ماژول‌ها scaffold دارند ولی عمق ۱۸ مرحله‌ای یکسان نیست.
